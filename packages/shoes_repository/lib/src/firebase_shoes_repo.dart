...
class FirebaseShoesRepo implements ShoesRepo {
  ...
  Future<List<Shoes>> getShoes() async {
    try {
      return await shoesCollection.get().then((QuerySnapshot<Map<String, dynamic>> value) => value.docs
          .map((QueryDocumentSnapshot<Map<String, dynamic>> e) => Shoes.fromEntity(ShoesEntity.fromDocument(e.data())))
          .toList(),);
    } catch (e) {
      log(e.toString());
      rethrow;
